  ...
  return (
    <aside className="fixed bottom-0 left-0 top-0 hidden max-h-screen min-h-screen min-w-[270px] max-w-[270px] flex-col bg-white px-6 py-[30px] lg:flex">
      ...
      <div className="mb-20 flex flex-1 flex-col">
        ...
        <Link className="mb-4 flex items-center gap-1 py-1 text-sm" to="/users">
          <LuUsers className="text-xl" />
          Users
        </Link>
        <Link className="mb-4 flex items-center gap-1 py-1 text-sm" to="/robots">
          <RiRobot2Line className="text-xl" />
          Robots
        </Link>
        <Link className="mb-4 flex items-center gap-1 py-1 text-sm" to="/weapons">
          <GiCrossedPistols className="text-xl" />
          Weapons
        </Link>
        <Link className="flex items-center gap-1 py-1 text-sm" to="/tasks">
          <GiCrossedPistols className="text-xl" />
          Tasks
        </Link>
      </div>
